...
fn get_distance_list_3(horizontal_positions: &Vec<u32>) -> Vec<i32> {
    let distance_list = (0..horizontal_positions.len())
        .map(|i| horizontal_positions.iter().map(|&val| i32::abs(i as i32 - val as i32)).sum())
        .collect::<Vec<_>>();
    distance_list
}
...
fn day_7_part_one() {
    ...
    //     }
    // }
    let (min_position, min_distance) = distance_list
        .iter()
        .enumerate()
        .min_by_key(|(_i, &v)| v)
        .map(|(i, &v)| (i, v))
    ...
    println!(
        "total fuel (min_distance): {}, min_position: {}",
