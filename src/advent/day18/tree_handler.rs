...
impl TreeNode {
    ...
    fn split(&mut self) -> bool {
        ...
        if let Some(value) = self.value {
            if value >= 10 {
                self.left_child.as_mut().unwrap().borrow_mut().value = Some(value / 2);
                self.right_child.as_mut().unwrap().borrow_mut().value = Some((value + 1) / 2);
                self.value = None;
                return true;
            }
            return false;
...
pub(crate) fn reduction(a: TreeNodePtr, b: TreeNodePtr) -> TreeNodePtr {
    let new_tree = TreeNode::add(a, b);
    // `reduce` should not return a value, it modifies the tree in place
    new_tree.borrow_mut().reduce();
    new_tree
}
