...
fn day_8_part_two() {
    ...
    let output_value_list = input_lines
        .iter()
        .enumerate()
    ...
    let output_number_list = output_value_list
        .iter()
        .map(|line| {
            let output_number = line.iter().rev().enumerate().fold(0, |acc, (i, v)| {
...
fn parse_word_len_five_or_six(
    ...
    //     }
    // }
    let output_value = bitmap_template
        .iter()
        .enumerate()
        .find(|(_, v)| **v == bitmap)
        .map(|(k, _)| k as u8)
