...
impl OperationEvalRecord {
    ...
    pub fn set_operator_packet_handler(&mut self,
        ...
        let get_subpackets_length_bits = |input_slice: &str| get_int_number(input_slice);

        //-- input_slice: 11 bits (# of sub-packets)
        let get_subpackets_count = |input_slice: &str| get_int_number(input_slice);
...
fn parse_packets(input_line: &str) -> (Vec<u32>, u128) {
    ...
    fn display_status_info(input_slice: &str, new_pos: usize, version: u32, type_id: u32, 
                            packet_count: &u32, operator_count: &u32, literal_count: &u32,) {
        match type_id {
            4 => {
                info!("");
    ...
    loop {
        ...
        display_status_info(input_slice, new_pos, version, type_id, &packet_count, &operator_count, &literal_count);
...
fn compute_stack_data(eval_rec_vec: &mut Vec<OperationEvalRecord>) {
    ...
    fn pop_top_record(eval_rec_vec: &mut Vec<OperationEvalRecord>, loop_count: &mut i32) -> Option<OperationEvalRecord> {
    ...
    fn pop_parent_record(eval_rec_vec: &mut Vec<OperationEvalRecord>, loop_count: &mut i32) -> Option<OperationEvalRecord> {
        match eval_rec_vec.pop() {
            Some(data_value) => {
                Some(data_value)
            },
...
fn get_packet_version(input_line: &str, start_pos: usize) -> u32 {
    get_int_number(&input_line[start_pos..start_pos+3])
}
...
fn get_packet_type_length_id(input_slice: &str, start_pos: usize) -> u32 {
    get_int_number(&input_slice[start_pos..start_pos+1])
}
