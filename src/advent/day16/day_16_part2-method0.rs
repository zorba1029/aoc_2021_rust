...
fn parse_packets(input_line: &str) -> (Vec<u32>, u128) {
    ...
    info!("[***] final_value = 🍏🍏 [{:?}] 🍏🍏", final_data.final_result);
...
fn compute_stack_data(op_status_vec: &mut Vec<OperatorPktStatus>) {
    ...
    while !op_status_vec.is_empty() {
        ...
        debug!("[STACK] ({}) ==[POP-1]==  TOP - {:#?}", loop_count, top_data);
        debug!("[STACK] ({}) ==[POP-2]==  PARENT - 🍒🍒 [BEFORE] - {:#?}", loop_count, parent_data);
        ...
        debug!("[STACK] ({}) ==[PUSH]== PARENT item - 🍏🍏 [AFTER] - {:#?}", loop_count, parent_data);
...
fn set_packet_status_op_type(input_slice: &str, new_pos: &usize, type_id: u32, op_status: &mut OperatorPktStatus) {
    ...
    match type_id {
        0 => {
            info!("[OP] TYPE ID = {}, |{}| => [SUM], start_pos = ({})", type_id, &input_slice[new_pos+3..new_pos+6], new_pos+3);
            op_status.set_op_type(OperatorType::SUM);
        },
        1 => {
            info!("[OP] TYPE ID = {}, |{}| => [PRODUCT], start_pos = ({})", type_id, &input_slice[new_pos+3..new_pos+6], new_pos+3);
            op_status.set_op_type(OperatorType::PRODUCT);
        },
        2 => {
            info!("[OP] TYPE ID = {}, |{}| => [MIN], start_pos = ({})", type_id, &input_slice[new_pos+3..new_pos+6], new_pos+3);
            op_status.set_op_type(OperatorType::MINIMUM);
        },
        3 => {
            info!("[OP] TYPE ID = {}, |{}| => [MAX], start_pos = ({})", type_id, &input_slice[new_pos+3..new_pos+6], new_pos+3);
            op_status.set_op_type(OperatorType::MAXIMUM);
        },
        4 => {
            info!("[OP] TYPE ID = |{}| -- DATA PACKET --, start_pos = ({})", &input_slice[new_pos+3..new_pos+6], new_pos+3);
        },
        5 => {
            info!("[OP] TYPE ID = {}, |{}| => [GT], start_pos = ({})", type_id, &input_slice[new_pos+3..new_pos+6], new_pos+3);
            op_status.set_op_type(OperatorType::GT);
        }, 
        6 => {
            info!("[OP] TYPE ID = {}, |{}| => [LT], start_pos = ({})", type_id, &input_slice[new_pos+3..new_pos+6], new_pos+3);
            op_status.set_op_type(OperatorType::LT);
        },
        7 => {
            info!("[OP] TYPE ID = {}, |{}| => [EQ], start_pos = ({})", type_id, &input_slice[new_pos+3..new_pos+6], new_pos+3);
            op_status.set_op_type(OperatorType::EQ);
        },
        _ => {
            info!("[OP] TYPE ID = {}, |{}| => UNKNOWN, start_pos = ({})", type_id, &input_slice[new_pos+3..new_pos+6], new_pos+3);
