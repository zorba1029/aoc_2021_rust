...
fn parse_packets(input_line: &str) -> (Vec<u32>, u128) {
    ...
    loop {
        ...
        if type_id != 4 {
            ...
            set_packet_status_op_type(input_slice, &new_pos, type_id, &mut eval_rec);

            debug!("[I]   Type_LEN_ID = {}, |{}|, start_pos = ({})", type_len_id, &input_slice[new_pos+6..new_pos+7], new_pos+6);
            operator_packet_handler(input_slice, &mut new_pos, &mut version_vec, &mut eval_rec);
            ...
            literal_packet_handler(input_slice, &mut new_pos, &mut version_vec, &mut eval_rec);
            compute_literal_packets_value(&mut eval_rec);
...
fn operator_packet_handler(input_slice: &str,
    ...
    match type_len_id {
        0 => {
            eval_rec.set_type_len_id(TypeLenID::L15);
            ...
            if t_start < current_input.len() && t_end < current_input.len() {
                ...
                *new_pos = *new_pos + 7 + 15;
                ...
                eval_rec.set_type_len_value(type_len_value);
            } else {
        ...
        1 => {
            eval_rec.set_type_len_id(TypeLenID::L11);
            let t_start = new_start+7;
...
fn literal_packet_handler(input_slice: &str,
    ...
    eval_rec.push_value(literal_single_value as u128);
    eval_rec.add_consumed_count(1);
    eval_rec.add_consumed_len(consumed_len);
    debug!("[DATA] Acc-Consumed LEN = {} (bits), Acc-Consumed COUNT = {}", eval_rec.consumed_len, eval_rec.consumed_count);
...
fn compute_literal_packets_value(eval_rec: &mut OperationEvalRecord) {
    //-------------------------------------------------------
    // compute_values - inner function
    ...
    //====================================================================
    info!("🍒[COMPUTE Literal Value]🍒 ----- {:?} -----", eval_rec.op_type);

    match eval_rec.op_type {
        OperatorType::SUM => {
            compute_values(eval_rec, |eval_rec: &mut OperationEvalRecord| {
        ...
        _ => {
            info!("[COMPUTE Literal Value] TYPE_ID: - Unknown ({:?}) ----", eval_rec.op_type);
        }
    }
}
