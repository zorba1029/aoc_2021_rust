...
fn get_subpacket_length_bits(input_slice: &str) -> u32 {
    ...
    debug!("         - sub-packet LEN (bits) = {}, subpacket_len_str = {}", subpacket_len_bits, input_slice);
...
fn get_subpacket_count(input_slice: &str) -> u32 {
    ...
    debug!("         - sub-packet COUNT = {}, subpacket_count_str = {}", subpacket_count, input_slice);

    subpacket_count
}

fn literal_packet_handler(input_slice: &str, new_pos: &mut usize, version_vec: &mut Vec<u32>) -> () {
    ...
    loop {
        debug!("       - [{}] LITERAL value(5 bits) = {} ({})", loop_count, literal_value, get_int_number(literal_value));
...
fn operator_packet_handler(input_slice: &str, new_pos: &mut usize, version_vec: &mut Vec<u32>) -> () {
    let new_start = *new_pos;
    let current_input = &input_slice; 
    let version = get_int_number(&current_input[new_start..new_start+3]);
    ...
    debug!("         operator packet: current input_len = {}, current_input = [{:?}]...", &current_input[new_start..].len(), &current_input[new_start..new_start+7]);

    match type_len_id {
        0 => {
            let t_start = new_start+7;
            let t_end = new_start+7+15;
            debug!("     [L] operator packet (I=0, L-15): ({},{}) = {}", t_start, t_end, t_end-t_start);
            if t_start < current_input.len() && t_end < current_input.len() {
                ...
                debug!("         - operator packet (I=0, L-15): total sub-packet size in BITS = {}", sub_packet_len);
        ...
        1 => {
            ...
            debug!("     [L] operator packet (I=1, L-11): ({},{}) = {}", t_start, t_end, t_end-t_start);
            if t_start < current_input.len() && t_end < current_input.len() {
                let sub_packet_count = get_subpacket_count(&current_input[t_start..t_end]);
                debug!("         - operator packet (I=1, L-11): # of sub-packets = {}", sub_packet_count);
