...
impl OperationEvalRecord {
    ...
    pub fn set_operator_packet_handler(
        ...
        let get_subpackets_count = |input_slice: &str| get_u32_number(input_slice);
            ...
            0 => {
                ...
                let type_len_value = if t_start < current_input.len() && t_end < current_input.len()
                {
                    ...
                    debug!(
                        ...
                        sub_packets_len,
                        &current_input[t_start..t_end],
                        t_start
                    );
                    *new_pos = *new_pos + 7 + 15;

                    TypeLenValue::L15Length {
    ...
    pub fn literal_packet_handler(
        ...
        debug!(
            "[DATA] Final Value = [{}], |{}|(len={} bits)",
            literal_single_value,
            literal_concat,
            literal_concat.len()
        );
        ...
        debug!(
            "[DATA] Acc-Consumed LEN = {} (bits), Acc-Consumed COUNT = {}",
            self.consumed_len, self.consumed_count
        );
        ...
        match self.op_type {
            OperatorType::SUM => {
                compute_values(self, |eval_rec: &mut OperationEvalRecord| {
                    ...
                    let sum: u128 = eval_rec.value_vec.iter().fold(0, |acc, &value| {
            ...
            OperatorType::PRODUCT => {
                compute_values(self, |eval_rec: &mut OperationEvalRecord| {
                    let product: u128 = eval_rec.value_vec.iter().fold(1, |acc, &value| {
            ...
            OperatorType::MINIMUM => {
                compute_values(self, |eval_rec: &mut OperationEvalRecord| {
                    ...
                    match min {
                        ...
                        None => {
                            warn!("[Literal Value] MIN - values are not available");
                        }
                    }
                });
            }
            OperatorType::MAXIMUM => {
                compute_values(self, |eval_rec: &mut OperationEvalRecord| {
                    ...
                    match max {
                        ...
                        None => {
                            warn!("[Literal Value] MAX - values are not available");
                        }
                    }
                });
            }
...
fn parse_packets(input_line: &str) -> (Vec<u32>, u128) {
    ...
    loop {
        ...
        let _type_len_id = get_packet_type_length_id(input_slice, new_pos + 6);
        ...
        //--------------------------------------
        // type ID == 4, then Literal packets
        match Some(type_id) {
            Some(type_id) if type_id != 4 => {
    ...
    info!(
        "[4] OUTER-MOST Final_value in final_data = 🍏🍏 [{:?}] 🍏🍏",
        final_data.final_result
    );
    info!(
        "[5] packet_count = {}, op = {}, literal = {},",
        packet_count, operator_count, literal_count
    );
...
fn compute_stack_data(eval_rec_vec: &mut Vec<OperationEvalRecord>) {
    ...
    while !eval_rec_vec.is_empty() {
        ...
        let top_data = match pop_top_record(eval_rec_vec, &mut loop_count) {
            Some(top_data) => top_data,
            None => return,
        };

        let mut parent_data = match pop_parent_record(eval_rec_vec, &mut loop_count) {
            Some(parent_data) => parent_data,
            None => {
                eval_rec_vec.push(top_data.to_owned());
                return;
            }
        };
        ...
        match parent_data.op_type {
            OperatorType::SUM => {
                compute_values(
                    ...
                    |parent_data: &mut OperationEvalRecord| {
                        ...
                        let sum: u128 = parent_data.value_vec.iter().fold(0, |acc, &value| {
            ...
            OperatorType::PRODUCT => {
                compute_values(
                    ...
                    top_data_length,
                    |parent_data: &mut OperationEvalRecord| {
                        let product: u128 = parent_data.value_vec.iter().fold(1, |acc, &value| {
            ...
            OperatorType::GT => {
                compute_values(
                    ...
                    &mut parent_data,
                    top_data_length,
                    |parent_data: &mut OperationEvalRecord| match (
                        parent_data.value_vec.first(),
                        parent_data.value_vec.get(1),
                    ) {
                        (Some(v1), Some(v2)) => {
                            if *v1 > *v2 {
...
fn get_u64_number(substr: &str) -> u64 {
    let num = substr.chars().rev().enumerate().fold(0, |acc, (i, c)| {
        acc + c.to_digit(2).unwrap() as u64 * u64::pow(2, i as u32)
    });
    num
}

fn get_packet_version(input_line: &str, start_pos: usize) -> u32 {
    get_u32_number(&input_line[start_pos..start_pos + 3])
}
...
fn get_packet_type_length_id(input_slice: &str, start_pos: usize) -> u32 {
    get_u32_number(&input_slice[start_pos..start_pos + 1])
}

//==========================================================
// Final Data and Value
//----------------------------------------------------------
//  INFO  advent_code_2021::advent::day16::day_16_part2 >
//  [2] OUTER-MOST Packet Data = 🍏🍏🍏🍏 OperationEvalRecord {
//     packet_seq: 1,
//     op_type: SUM,
//     start_input_pos: 0,
//     type_len_id: L11,
//     type_len_value: L11Count {
...
//     finalized: true,
// } 🍏🍏🍏🍏
//  INFO  advent_code_2021::advent::day16::day_16_part2 > [3] OUTER-MOST Values in final_data = [0, 0, 0, 47144, 238, 87, 196, 657596, 9, 3, 0, 15811746, 12920678784, 0, 0, 18432, 172, 27, 0, 56, 339840, 20925, 0, 6, 43, 375791589211, 176, 0, 12, 20626, 5, 175, 0, 10, 3976415, 840, 0, 53086862385, 729288, 1058775397181, 369780, 118, 19911, 2, 15, 26058, 7, 48428531259, 136, 0, 1747, 1189708, 2517] 🍏🍏🍏🍏
//  INFO  advent_code_2021::advent::day16::day_16_part2 > [4] OUTER-MOST Final_value in final_data = 🍏🍏 [1549026292886] 🍏🍏
//  INFO  advent_code_2021::advent::day16::day_16_part2 > [5] packet_count = 270, op = 101, literal = 170,
//  INFO  advent_code_2021::advent::day16::day_16_part2 > [**] version all items = [6, 3, 6, 0, 5, 7, 3, 7, 2, 4, 5, 4, 3, 6, 6, 0, 2, 6, 3, 7, 1, 0, 1, 3, 5, 2, 0, 6, 0, 2, 7, 7, 3, 1, 5, 1, 0, 7, 1, 7, 1, 2, 6, 1, 6, 2, 6, 0, 6, 2, 7, 5, 3, 0, 3, 3, 6, 4, 0, 7, 2, 3, 7, 6, 6, 7, 0, 0, 7, 0, 7, 5, 1, 6, 6, 2, 5, 6, 1, 3, 4, 7, 7, 2, 7, 2, 7, 5, 7, 7, 6, 6, 2, 7, 6, 4, 3, 4, 1, 2, 1, 7, 0, 1, 2, 6, 4, 0, 4, 0, 0, 3, 7, 1, 1, 0, 2, 6, 6, 5, 0, 3, 6, 5, 6, 6, 0, 4, 1, 7, 1, 2, 5, 4, 5, 2, 6, 6, 5, 5, 3, 1, 0, 7, 3, 6, 6, 3, 5, 7, 3, 1, 2, 0, 0, 4, 2, 4, 0, 7, 2, 4, 2, 2, 5, 3, 5, 5, 5, 0, 5, 5, 7, 6, 2, 3, 5, 1, 2, 6, 1, 7, 4, 2, 3, 2, 1, 0, 0, 1, 5, 7, 2, 2, 5, 6, 0, 7, 0, 6, 6, 5, 1, 6, 1, 1, 2, 3, 1, 6, 7, 2, 5, 0, 4, 3, 5, 5, 0, 4, 3, 1, 7, 1, 4, 5, 4, 6, 2, 5, 7, 1, 3, 5, 2, 3, 6, 1, 0, 7, 2, 0, 4, 7, 2, 0, 4, 4, 3, 1, 1, 3, 0, 7, 6, 1, 6, 3, 5, 6, 3, 4, 0, 7, 6, 1, 5, 7, 0]
//  INFO  advent_code_2021::advent::day16::day_16_part2 > [**] part-1: version sum = 963
//  INFO  advent_code_2021::advent::day16::day_16_part2 > [**] part-2: final value = 1549026292886
// RUST_LOG=debug cargo run --release  0.06s user 0.03s system 28% cpu 0.319 total
// =================================================================================
//  [ZORBA] 🔥  /Volumes/SSD_01/zorba/fun/rust-lang-study/advent_code/advent_code_2021   master ±✚
