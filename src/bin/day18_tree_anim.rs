// Day 18: Snailfish — reduction shown as BOTH the flat string and the tree,
// animating together (in-terminal).
//
//   cargo run --release --bin day18_tree_anim                 # sample-1, all lines
//   cargo run --release --bin day18_tree_anim -- sample-2     # the other sample
//   cargo run --release --bin day18_tree_anim -- input 5      # real input, first 5 lines
//
// Same reduction engine as day18_anim; each frame draws the flat colored string
// and, below it, the same number as an indented binary tree.
//
